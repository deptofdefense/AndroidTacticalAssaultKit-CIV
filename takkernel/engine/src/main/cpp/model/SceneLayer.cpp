...
namespace
{
    ...
    TAKErr writeZipCommentInfo(const SceneInfo &info, const atakmap::util::AttributeSet &metadata) NOTHROWS;
...
TAKErr SceneLayer::add(int64_t *sid, const std::shared_ptr<Scene> &scene, const SceneInfo &info) NOTHROWS
{
    ...
    if (info.aabb.get()) {
        ...
        code = Mesh_transform(&aabb_wgs84, *info.aabb, MeshTransformOptions(info.srid, localFrame), MeshTransformOptions(4326));
        TE_CHECKRETURN_CODE(code);
    } else if (info.location.get()) {
        ...
        aabb_wgs84.minX = info.location->longitude - padX;
        aabb_wgs84.minY = info.location->latitude - padY;
        aabb_wgs84.maxX = info.location->longitude + padX;
        aabb_wgs84.maxY = info.location->latitude + padY;
    } else {
    ...
    ls.addPoint(aabb_wgs84.minX, aabb_wgs84.maxY);
    ls.addPoint(aabb_wgs84.maxX, aabb_wgs84.maxY);
    ls.addPoint(aabb_wgs84.maxX, aabb_wgs84.minY);
    ls.addPoint(aabb_wgs84.minX, aabb_wgs84.minY);
    ls.addPoint(aabb_wgs84.minX, aabb_wgs84.maxY);
        
    auto location = GeometryPtr(new atakmap::feature::Polygon(ls), Memory_deleter_const<atakmap::feature::Geometry>);
...
TAKErr SceneLayer::update(const int64_t sid, const SceneInfo &info, const atakmap::util::AttributeSet &metadata) NOTHROWS
{
    TAKErr code(TE_Ok);
    Lock lock(mutex_);
    code = lock.status;
    ...
    auto geom = GeometryPtr(new atakmap::feature::Polygon(ls), Memory_deleter_const<atakmap::feature::Geometry>);
...
TAKErr SceneLayer::getMetadata(atakmap::util::AttributeSet &metadata, const char *uri) const NOTHROWS
{
        ...
        TE_BEGIN_TRAP() {
            ...
            if (zipComment == "") {
                return TE_Ok;  // No comment, no metadata, no error.
            }
...
namespace
{
    ...
    TAKErr writeZipCommentInfo(const SceneInfo& info, const atakmap::util::AttributeSet &metadata) NOTHROWS {
        ...
        bool exists = false;
        IO_existsV(&exists, info.uri);

        if (!exists)
            return TE_Err;

        TE_BEGIN_TRAP() {
            ZipCommentInfoPtr existingZipCommentInfo(nullptr, nullptr);

            String zipComment;
            code = IO_getZipComment(zipComment, info.uri);
            TE_CHECKRETURN_CODE(code);

            if (zipComment != "") {
                // ignore the return code
                ZipCommentInfo::Create(existingZipCommentInfo, zipComment.get());
            }

            ZipCommentInfoPtr newZipCommentInfo(nullptr, nullptr);
            code = ZipCommentInfo::Create(newZipCommentInfo);
            TE_CHECKRETURN_CODE(code);

            newZipCommentInfo->SetProjectionSrid(info.srid);
            newZipCommentInfo->SetAltitudeMode(info.altitudeMode);
            newZipCommentInfo->SetLocalFrame(*info.localFrame.get());
            newZipCommentInfo->SetDisplayResolutions(info.maxDisplayResolution, info.minDisplayResolution);
            newZipCommentInfo->SetResolution(info.resolution);
            if (info.aabb.get())
                newZipCommentInfo->SetEnvelope(*info.aabb.get());
            if (info.location.get())
                newZipCommentInfo->SetLocation(*info.location.get());
            newZipCommentInfo->SetMetadata(metadata);

            if (existingZipCommentInfo == nullptr ||
                *existingZipCommentInfo != *newZipCommentInfo) {
                code = newZipCommentInfo->ToString(zipComment);
                TE_CHECKRETURN_CODE(code);

                code = IO_setZipComment(info.uri, zipComment);
                TE_CHECKRETURN_CODE(code);
            }
        } TE_END_TRAP(code);
