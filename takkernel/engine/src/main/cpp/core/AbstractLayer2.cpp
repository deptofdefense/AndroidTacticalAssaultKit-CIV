...
void AbstractLayer2::registerExtension(const char *extensionName, ExtensionPtr &&extension) NOTHROWS
{
    if (!extensionName)
    {
        Logger_log(TELL_Warning, "%s: Failed to register extension with NULL name", this->name_.get());
        return;
    }
    if (!extension.get())
    {
        Logger_log(TELL_Warning, "%s: Failed to register NULL extension, %s", this->name_.get(), extensionName);
        return;
    }
