    ...
    xray_color_(0x3F07FFFFu) 
{
    ...
    if (info.aabb) {
        do {
            ...
            Matrix2 localFrame;
            if (info_.localFrame)
                localFrame = *info_.localFrame;
            if (Mesh_transform(&aabb_wgs84, *info_.aabb, MeshTransformOptions(info_.srid, localFrame), MeshTransformOptions(4326)) != TE_Ok)
                break;
            double pts[10] =
            {
                aabb_wgs84.minX, aabb_wgs84.minY,
                aabb_wgs84.minX, aabb_wgs84.maxY,
                aabb_wgs84.maxX, aabb_wgs84.maxY,
                aabb_wgs84.maxX, aabb_wgs84.minY,
                aabb_wgs84.minX, aabb_wgs84.minY,
            };
            LineString2 ls;
            ls.addPoints(pts, 5u, 2u);
            indicator.setBounds(ls, atakmap::feature::BasicStrokeStyle(0xFF00FF00, 1.0), true);
        } while (false);
    }
}
