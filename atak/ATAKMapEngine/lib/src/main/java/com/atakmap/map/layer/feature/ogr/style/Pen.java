...
public class Pen implements DrawingTool {
    ...
    public void pushParam(String key, String value) {
        if (key.equals(PARAM_PEN_COLOR)) {
            this.color = FeatureStyleParser.parseOgrColor(value);
        } else if (key.equals(PARAM_PEN_WIDTH)) {
            if (value.endsWith("px"))
                this.width = Float.valueOf(value.substring(0, value.length() - 2));
            // XXX - other units
        } else if (key.equals(PARAM_PATTERN)) {
            // XXX -
        } else if (key.equals(PARAM_PEN_NAME)) {
            // XXX -
        } else if (key.equals(PARAM_PEN_CAP)) {
            if (value.equals("b"))
                this.cap = CAP_BUTT;
        ...
        } else if (key.equals(PARAM_PEN_JOIN)) {
            if (value.equals("m"))
        ...
        } else if (key.equals(PARAM_PERPENDICULAR_OFFSET)) {
