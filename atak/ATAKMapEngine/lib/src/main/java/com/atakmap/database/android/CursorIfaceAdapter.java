...
public final class CursorIfaceAdapter extends AbstractWindowedCursor {
    ...
    public boolean onMove(int oldPosition, int newPosition) {
        ...
        if (newPosition >= (mWindow.getStartPosition() + mWindow.getNumRows())) {
        ...
        for (int j = 0; j < this.getColumnCount(); j++) {
            switch (this.iface.getType(j)) {
                case CursorIface.FIELD_TYPE_BLOB:
                    this.mWindow.putBlob(this.iface.getBlob(j), this.cursorPos, j);
                    break;
                case CursorIface.FIELD_TYPE_FLOAT:
                    this.mWindow.putDouble(this.iface.getDouble(j), this.cursorPos, j);
                    break;
                case CursorIface.FIELD_TYPE_INTEGER:
                    this.mWindow.putLong(this.iface.getLong(j), this.cursorPos, j);
                    break;
                case CursorIface.FIELD_TYPE_NULL:
                    this.mWindow.putNull(this.cursorPos, j);
                    break;
                case CursorIface.FIELD_TYPE_STRING:
                    this.mWindow.putString(this.iface.getString(j), this.cursorPos, j);
                    break;
            }
        }
