...
public class GLQuadTileNode2 implements 
    ...
    private final static int STATE_RESOLVED = 0x01;
    private final static int STATE_RESOLVING = 0x02;
    private final static int STATE_UNRESOLVED = 0x04;
    private final static int STATE_UNRESOLVABLE = 0x08;
    private final static int STATE_SUSPENDED = 0x10;
    
    private final static int[] POI_ITERATION_BIAS = 
        {
            1, 3, 0, 2, // 0
            1, 0, 2, 3, // 1
            0, 1, 2, 3, // 2
    ...
    private final static Comparator<GLQuadTileNode2> ORPHANING_COMPARATOR = new Comparator<GLQuadTileNode2>() {
        @Override
    ...
    private final static Comparator<PointL> POINT_COMPARATOR = new Comparator<PointL>() {
        @Override
    ...
    static double tileDistanceLevelAdjust = 1.0d;
    static boolean mipmapEnabled = false;


    private final static ElevationManager.QueryParameters ELEVATION_QUERY_TERRAIN;
    static {
        ELEVATION_QUERY_TERRAIN = new ElevationManager.QueryParameters();
        ELEVATION_QUERY_TERRAIN.elevationModel = ElevationData.MODEL_TERRAIN;
    }
    ...
    //protected final static boolean HARDWARE_TRANSFORMS = false;
    ...
    private void draw(GLMapView view, int level, long srcX, long srcY, long srcW, long srcH, long poiX, long poiY, boolean cull) {
        ...
        if (this.level == level) {
            ...
            if(this.core.tileReader.isMultiResolution()) {
                ...
                if(unresolvedChildren && ((this.state != State.UNRESOLVED) || this.receivedUpdate)) {
                    // force child to re-borrow
                    for (int i = 0; i < this.children.length; i++) {
                        if(this.children[i] != null && this.children[i].state == State.UNRESOLVABLE) {
                            if(this.children[i].borrowingFrom != null && this.children[i].borrowingFrom != this) {
            ...
            for (int j = 0; j < this.children.length; j++) {
                final int i = POI_ITERATION_BIAS[(iterOffset*4)+j];
                if (visibleChildren[i]) {
                    this.children[i].verticesInvalid = this.verticesInvalid;
                    this.children[i].draw(view, level, srcX, srcY, srcW, srcH, poiX, poiY, cull);
                    visibleChildren[i] = false;
    ...
    public State getState() {
        ...
        if(MathUtils.hasBits(mask, STATE_RESOLVING) || this.core.progressiveLoading)
    ...
    public void requestUpdate(final int id, byte[] data, final int dstX, final int dstY,
        ...
        final Buffer buf = GdalGraphicUtils.createBuffer(data,
                dstW,
                dstH,
        ...
        this.view.getSurface().queueEvent(new Runnable() {
            @Override
            public void run() {
                if (!checkRequest(id))
                    return;
    ...
    public synchronized void dispose() {
        if(this.root == this && !this.core.disposed) {
            this.core.init.dispose(this.core.initResult);
    ...
    protected static interface VertexResolver extends Releasable {
    ...
    protected class PreciseVertexResolver extends DefaultVertexResolver implements Runnable {
        ...
        public PreciseVertexResolver(Object syncOn) {
            ...
            this.pending = new TreeSet<PointL>(POINT_COMPARATOR);
            this.precise = new TreeMap<PointL, GeoPoint>(POINT_COMPARATOR);
            this.unresolvable = new TreeSet<PointL>(POINT_COMPARATOR);

            this.currentRequest = new TreeSet<PointL>(POINT_COMPARATOR);
            ...
            if (!view.targeting && this.numNodesPending == 0 && this.view != null) {
                ...
                for (final GLQuadTileNode2 node : this.requestNodes) {
                    ...
                    if (node.glTexGridWidth < targetGridWidth)
                        view.getSurface().queueEvent(new Runnable() {
                            @Override
                            public void run() {
                                if (node.glTexGridWidth < targetGridWidth) {
                                    node.expandTexGrid();
        ...
        public void project(GLMapView view, long imgSrcX, long imgSrcY, GridVertex retval) {
            ...
            if (!view.targeting) {
                ...
                synchronized (this.syncOn) {
                    ...
                    if (geo == null) {
                        if(view.rigorousRegistrationResolutionEnabled)
                            this.resolve();
    ...
    /**
     * The core properties/state for a given quadtree.  The root node is
     * considered the owner.
     * 
     * @author Developer
     */
    protected static class NodeCore {
        private final Initializer init;
        private Initializer.Result initResult;
        ...
        public final int srid;
        public VertexResolver vertexResolver;
        public GLTextureCache textureCache;
        ...
        private static NodeCore create(ImageInfo info, TileReaderFactory.Options readerOpts, Options opts, boolean throwOnReaderFailedInit, Initializer init) {
            Initializer.Result result = init.init(info, readerOpts);
