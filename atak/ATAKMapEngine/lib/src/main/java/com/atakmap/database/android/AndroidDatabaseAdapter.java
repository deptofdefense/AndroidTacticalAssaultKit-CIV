...
public final class AndroidDatabaseAdapter implements DatabaseIface {
    ...
    public StatementIface compileStatement(String sql) {
        ...
        return new AndroidStatementAdapter(this.impl.compileStatement(sql), type);
    }
    ...
    public static AndroidDatabaseAdapter openOrCreateDatabase(String path) {
        return openDatabase(path, SQLiteDatabase.OPEN_READWRITE|SQLiteDatabase.CREATE_IF_NECESSARY);
    }
    ...
    public static int update(DatabaseIface db, String table, ContentValues contentValues,
            String where, String[] selectionArgs) {
        if (db instanceof AndroidDatabaseAdapter) {
            return ((AndroidDatabaseAdapter) db).impl.update(table, contentValues, where,
                    selectionArgs);
        }
    ...
    public static int delete(DatabaseIface db, String table, String where, String[] selectionArgs) {
        if (db instanceof AndroidDatabaseAdapter) {
            return ((AndroidDatabaseAdapter) db).impl.delete(table, where, selectionArgs);
        }
    ...
    public static long insert(DatabaseIface db, String table, String nullColumnHack,
            ContentValues values) {
        if (db instanceof AndroidDatabaseAdapter) {
            return ((AndroidDatabaseAdapter) db).impl.insert(table, nullColumnHack, values);
        }

        if (values == null || values.size() < 1)
            return 0L;

        Object[] args = new String[values.size()];
    ...
    public static Cursor query(SQLiteQueryBuilder queryBuilder, DatabaseIface db,
            String[] projection, String selection, String[] selectionArgs, String groupBy,
            String having, String sortOrder) {
        return query(queryBuilder, db, projection, selection, selectionArgs, groupBy, having,
                sortOrder, null);
    }

    public static Cursor query(SQLiteQueryBuilder queryBuilder, DatabaseIface db, String[] projection,
            String selection, String[] selectionArgs, String groupBy, String having,
            String sortOrder, String limit) {
        if (db instanceof AndroidDatabaseAdapter) {
            return queryBuilder.query(((AndroidDatabaseAdapter) db).impl, projection, selection,
                    selectionArgs, groupBy, having, sortOrder, limit);
        }

        final String sql = queryBuilder.buildQuery(projection, selection, groupBy, having,
                sortOrder, limit);
