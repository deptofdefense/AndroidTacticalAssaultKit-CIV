...
public class GLES20FixedPipeline extends GLES30 {
    ...
    private static int getGenericVectorProgram(int size, boolean colorPointer) {
        final int colorFlags = colorPointer ? 0x00 : 0x01;
        final int programFlags = colorFlags | (size << 1);
        Program p = vectorPrograms.get(Integer.valueOf(programFlags));
        if (p == null) {
            p = new Program();
            ...
            p.create(vertShaderSource, fragShaderSource);
            vectorPrograms.put(Integer.valueOf(programFlags), p);
        }
        return p.program;
    }

    private static int getGenericTextureProgram(int vSize, int tcSize, boolean colorize,
            boolean point) {
        if (tcSize != 2 || vSize < 0 || vSize > 3)
            throw new UnsupportedOperationException();
        final int colorFlag = colorize ? 0x01 : 0x00;
        final int pointFlag = point ? 0x01 : 0x00;
        final int programFlags = colorFlag | (pointFlag << 1) | (vSize << 2) | (tcSize << 4);
        Program p = texturePrograms.get(Integer.valueOf(programFlags));
        if (p == null) {
            p = new Program();
            String vertShaderSource;
            switch (vSize) {
                case 2:
                    if (point)
                        ...
                        vertShaderSource = TEXTURE_2D_VERT_SHADER_SRC;
                    break;
                case 3:
                    if (point)
                        vertShaderSource = POINT_3D_VERT_SHADER_SRC;
                    else
                        vertShaderSource = TEXTURE_3D_V2_VERT_SHADER_SRC;
                    break;
                default:
                    throw new UnsupportedOperationException();
            }
            ...
            if (point)
                ...
                fragShaderSource = MODULATED_TEXTURE_FRAG_SHADER_SRC;
            else
                fragShaderSource = GENERIC_TEXTURE_FRAG_SHADER_SRC;

            p.create(vertShaderSource, fragShaderSource);
            texturePrograms.put(Integer.valueOf(programFlags), p);
        }
        return p.program;
    }
    ...
    private static void drawGenericVector(int mode, int first, int vertexCount,
        ...
        final int programHandle = getGenericVectorProgram(vPointer.size, false);
        GLES30.glUseProgram(programHandle);

        final int uProjectionHandle = GLES30.glGetUniformLocation(programHandle, "uProjection");
        GLES30.glUniformMatrix4fv(uProjectionHandle, 1, false, projection.current, 0);

        final int uModelViewHandle = GLES30.glGetUniformLocation(programHandle, "uModelView");
        GLES30.glUniformMatrix4fv(uModelViewHandle, 1, false, modelView.current, 0);

        final int aVertexCoordsHandle = GLES30.glGetAttribLocation(programHandle, "aVertexCoords");
    ...
    private static void drawGenericVector(int mode, int first, int vertexCount,
        ...
        final int programHandle = getGenericVectorProgram(vPointer.size, true);
        GLES30.glUseProgram(programHandle);
        ...
        final int aColorPointerHandle = GLES30.glGetAttribLocation(programHandle, "aColorPointer");
    ...
    private static void drawBoundTexture(int mode, int textureUnit, int first, int vertexCount,
        ...
        float alpha = a * aMod;
        final boolean colorize = (r != 1.0f || g != 1.0f || b != 1.0f || alpha != 1.0f);
        final int programHandle = getGenericTextureProgram(vertexCoords.size, textureCoords.size,
                colorize, mode == GL_POINTS);
        GLES30.glUseProgram(programHandle);
        ...
        if(vertexCoords.size == 3) {
            final int uTextureMatrixHandle = GLES30.glGetUniformLocation(programHandle, "uTextureMx");
        ...
        if (mode == GLES30.GL_POINTS) {
            final int aPointSizeHandle = GLES30.glGetUniformLocation(programHandle, "aPointSize");
            GLES30.glUniform1f(aPointSizeHandle, pointSize);
        }

        final int aVertexCoordsHandle = GLES30.glGetAttribLocation(programHandle, "aVertexCoords");
        final int aTextureCoordsHandle = GLES30
                .glGetAttribLocation(programHandle, "aTextureCoords");
