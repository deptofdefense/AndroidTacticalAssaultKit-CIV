...
package com.atakmap.android.control;
 ...
 * {@link MotionEvent}s. The {@link OnScaleGestureListener} callback will notify users when a
 * particular gesture event has occurred. This class should only be used with {@link MotionEvent}s
 * reported via touch. To use this class:
 * <ul>
 * <li>Create an instance of the {@code ScaleGestureDetector} for your {@link View}
...
public class ScaleGestureDetector {
     ...
     * the different gestures then implement this interface. If you only want to listen for a subset
     * it might be easier to extend {@link SimpleOnScaleGestureListener}. An application will
     * receive events in the following order:
     * <ul>
    ...
    public interface OnScaleGestureListener {
         ...
         *         is handled. This can be useful if an application, for example, only wants to
         *         update scaling factors if the change is greater than 0.01.
         */
         ...
         *         example, if a gesture is beginning with a focal point outside of a region where
         *         it makes sense, onScaleBegin() may return false to ignore the rest of the
         *         gesture.
         */
        boolean onScaleBegin(ScaleGestureDetector detector);

        /**
         * Responds to the end of a scale gesture. Reported by existing pointers going up. Once a
         * scale has ended, {@link ScaleGestureDetector#getFocusX()} and
         * {@link ScaleGestureDetector#getFocusY()} will return the location of the pointer
     ...
     * events. This implements all methods in {@link OnScaleGestureListener} but does nothing.
     * {@link OnScaleGestureListener#onScale(ScaleGestureDetector)} returns {@code false} so that a
     * subclass can retrieve the accumulated scale factor in an overridden onScaleEnd.
    ...
    public ScaleGestureDetector(Context context,
        ...
        if (model.equals("PH-1")) {
            Log.d(TAG, "pressure hack applied for the PH-1");
            PRESSURE_THRESHOLD = 0.23f;
        }
    }
        ...
        if (!mGestureInProgress) {
            switch (action & MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mSloppyGesture && (event.getPointerCount() > 1)) {
    ...
    public double getAngle() {
        float deltaX = getRawX(mStartEvent, 1) - getRawX(mStartEvent, 0) +
                getRawX(mCurrEvent, 1) - getRawX(mCurrEvent, 0);
        float deltaY = getRawY(mStartEvent, 1) - getRawY(mStartEvent, 0) +
                getRawY(mCurrEvent, 1) - getRawY(mCurrEvent, 0);
     ...
     * focal point is directly between the two pointers forming the gesture. If a gesture is ending,
     * the focal point is the location of the remaining pointer on the screen. If
     * {@link #isInProgress()} would return false, the result of this function is undefined.
     * 
