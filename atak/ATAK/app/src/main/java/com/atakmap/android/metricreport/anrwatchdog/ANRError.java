...
package com.atakmap.android.metricreport.anrwatchdog;
...
/**
 * Error thrown by {@link com.atakmap.android.metricreport.anrwatchdog.ANRWatchDog} when an ANR is detected.
 ...
 */
@SuppressWarnings({
        "Convert2Diamond", "UnusedDeclaration"
})
public class ANRError extends Error {
    ...
    static ANRError New(String prefix, boolean logThreadsWithoutStackTrace) {
        ...
        $._Thread tst = null;
        for (Map.Entry<Thread, StackTraceElement[]> entry : stackTraces
                .entrySet())
            tst = new $(getThreadTitle(entry.getKey()),
                    entry.getValue()).new _Thread(tst);
    ...
    static ANRError NewMainOnly() {
        ...
        return new ANRError(
                new $(getThreadTitle(mainThread), mainStackTrace).new _Thread(
                        null));
    }
