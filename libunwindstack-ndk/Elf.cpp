...
void Elf::CacheAdd(MapInfo* info) {
  ...
  if (info->offset != 0) {
    ...
    (*cache_)[info->name + ':' + unwindpriv::to_string(info->offset)] =
...
bool Elf::CacheAfterCreateMemory(MapInfo* info) {
  ...
  (*cache_)[info->name + ':' + unwindpriv::to_string(info->offset)] = std::make_pair(info->elf, true);
  return true;
}
...
bool Elf::CacheGet(MapInfo* info) {
  ...
  if (info->offset != 0) {
    name += ':' + unwindpriv::to_string(info->offset);
  }
