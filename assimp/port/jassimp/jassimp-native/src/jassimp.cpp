...
#define lprintf(...) __android_log_print(ANDROID_LOG_VERBOSE, "ASSIMP", __VA_ARGS__)
#else
...
static jint callI(JNIEnv *env, jobject object, const char* typeName, const char* methodName,
                   const char* signature, jvalue* params)
{
    jclass clazz = env->FindClass(typeName);
    SmartLocalRef clazzRef(env, clazz);
    ...
    lprintf("calling %s", methodName);
    return env->CallIntMethodA(object, mid, params);
}

static jlong callJ(JNIEnv *env, jobject object, const char* typeName, const char* methodName,
                 const char* signature, jvalue* params)
{
    jclass clazz = env->FindClass(typeName);
    SmartLocalRef clazzRef(env, clazz);
    ...
    lprintf("calling %s", methodName);
    return env->CallLongMethodA(object, mid, params);
}
                   ...
                   const char* signature)
{
    ...
    lprintf("calling %s", methodName);
    return env->CallLongMethod(object, mid);
}
...
class JavaIOStream2 : public Assimp::IOStream {
private:
    JNIEnv* mJniEnv;
    jobject jIOStream;
    
public:
    JavaIOStream2(JNIEnv *env, jobject jIOStream) :
    mJniEnv(env),
    jIOStream(env->NewGlobalRef(jIOStream))
    {};
    
    JavaIOStream2(void) {
        mJniEnv->DeleteGlobalRef(jIOStream);
    };
    
    size_t Read(void* pvBuffer, size_t pSize, size_t pCount)
    {
        jobject javaBuffer = mJniEnv->NewDirectByteBuffer(pvBuffer, pSize * pCount);
    
        jvalue params[1];
        params[0].l = javaBuffer;
    
        if (!call(mJniEnv, jIOStream, "jassimp/AiIOStream", "read", "(Ljava/nio/ByteBuffer;)Z", params)) {
            return 0;
        }
    
        size_t cnt = calli(mJniEnv, javaBuffer, "java/nio/ByteBuffer", "position", "()I");
        return cnt / pSize;
    };
    ...
    aiReturn Seek(size_t pOffset, aiOrigin pOrigin)
    {
        ...
        jlong pos = callJ(mJniEnv, jIOStream, "jassimp/AiIOStream", "seek", "(JI)J", params);
        if (pos >= 0) {
            result = AI_SUCCESS;
        }
    ...
    size_t Tell(void) const
    {
        jlong result = callJ(mJniEnv, jIOStream, "jassimp/AiIOStream", "tell", "()J");
        return result;
    };
    
    size_t FileSize() const
    {
        jlong result = calli(mJniEnv, jIOStream, "jassimp/AiIOStream", "getFileSize", "()I");
        return result;
    };
...
class JavaIOSystem2 : public Assimp::IOSystem {
	private:
    JNIEnv* mJniEnv;
	jobject mJavaIOSystem;
	
	public:
	JavaIOSystem2(JNIEnv* env, jobject& javaIOSystem) :
		mJniEnv(env),
		mJavaIOSystem(env->NewGlobalRef(javaIOSystem))
	{};
    
    ~JavaIOSystem2() {
        mJniEnv->DeleteGlobalRef(mJavaIOSystem);
    }
    ...
    Assimp::IOStream* Open(const char* pFile,const char* pMode = "rb")
    {
        ...
        return new JavaIOStream2(mJniEnv, jStream);
    };
    void Close( Assimp::IOStream* pFile)
    {
		jvalue params[1];
		params[0].l = ((JavaIOStream2*) pFile)->javaObject();
		callv(mJniEnv, mJavaIOSystem, "jassimp/AiIOSystem", "close", "(Ljassimp/AiIOStream;)V", params);
    	delete pFile;
    };
};
  ...
  (JNIEnv *env, jclass jClazz, jstring jFilename, jlong postProcess, jobject ioSystem)
{
	...
	if(ioSystem != NULL)
    {
		imp.SetIOHandler(new JavaIOSystem2(env, ioSystem));
		lprintf("Created aiFileIO\n");
	}
	
	lprintf("opening file: '%s' post process %d \n", cFilename, (unsigned int)postProcess);
