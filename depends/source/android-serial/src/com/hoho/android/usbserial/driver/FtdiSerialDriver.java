...
public class FtdiSerialDriver implements UsbSerialDriver {
    ...
    private class FtdiSerialPort extends CommonUsbSerialPort {
        ...
        public static final int FTDI_DEVICE_OUT_REQTYPE =
                UsbConstants.USB_TYPE_VENDOR | USB_RECIP_DEVICE
                        | USB_ENDPOINT_OUT;
        ...
        private final int filterStatusBytes(byte[] src, byte[] dest,
            ...
            final int packetsCount = totalBytesRead / maxPacketSize
                    + (totalBytesRead % maxPacketSize == 0 ? 0 : 1);
        ...
        public void open(UsbDeviceConnection connection) throws IOException {
            ...
            try {
                for (int i = 0; i < mDevice.getInterfaceCount(); i++) {
                    if (connection
                            .claimInterface(mDevice.getInterface(i), true)) {
        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            ...
            if (ENABLE_ASYNC_READS) {
                ...
                synchronized (mReadBufferLock) {
                    final int readAmt = Math.min(dest.length,
                            mReadBuffer.length);
        ...
        private int setBaudRate(int baudRate) throws IOException {
            ...
            if (result != 0) {
                throw new IOException("Setting baudrate failed: result="
                        + result);
            }
        ...
        public void setParameters(int baudRate, int dataBits, int stopBits,
            ...
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    ...
                    null, 0, USB_WRITE_TIMEOUT_MILLIS);
            if (result != 0) {
                throw new IOException("Setting parameters failed: result="
                        + result);
            }
        }
        ...
        public boolean purgeHwBuffers(boolean purgeReadBuffers,
            ...
            if (purgeReadBuffers) {
                int result = mConnection.controlTransfer(
                        ...
                        SIO_RESET_PURGE_RX, 0 /* index */, null, 0,
                        USB_WRITE_TIMEOUT_MILLIS);
                if (result != 0) {
                    throw new IOException("Flushing RX failed: result="
                            + result);
                }
            }
            ...
            if (purgeWriteBuffers) {
                int result = mConnection.controlTransfer(
                        FTDI_DEVICE_OUT_REQTYPE, SIO_RESET_REQUEST,
                        SIO_RESET_PURGE_TX, 0 /* index */, null, 0,
                        USB_WRITE_TIMEOUT_MILLIS);
                if (result != 0) {
                    throw new IOException("Flushing RX failed: result="
                            + result);
    ...
    public static Map<Integer, int[]> getSupportedDevices() {
        ...
        supportedDevices.put(Integer.valueOf(UsbId.VENDOR_SEALEVEL_SYSTEMS),
                new int[] {
                        UsbId.SEALINK_422,
                });
        supportedDevices.put(Integer.valueOf(UsbId.VENDOR_WHOI),
                new int[] {
                        UsbId.WHOI_RADIO,
                });
