...
public class ProlificSerialDriver implements UsbSerialDriver {
    ...
    class ProlificSerialPort extends CommonUsbSerialPort {
        ...
        private int mBaudRate = -1, mDataBits = -1, mStopBits = -1,
                mParity = -1;
        ...
        private final byte[] inControlTransfer(int requestType, int request,
            ...
            int result = mConnection.controlTransfer(requestType, request,
                    value,
        ...
        private final void ctrlOut(int request, int value, int index,
                byte[] data)
                throws IOException {
            outControlTransfer(PROLIFIC_CTRL_OUT_REQTYPE, request, value,
                    index,
                    data);
        }
        ...
        private final void readStatusThreadFunction() {
            try {
                while (!mStopReadStatusThread) {
                    ...
                    int readBytesCount = mConnection.bulkTransfer(
                            mInterruptEndpoint,
                            buffer,
                            STATUS_BUFFER_SIZE,
                            500);
            ...
            if ((mReadStatusThread == null) && (mReadStatusException == null)) {
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread == null) {
                        byte[] buffer = new byte[STATUS_BUFFER_SIZE];
                        int readBytes = mConnection.bulkTransfer(
                                mInterruptEndpoint,
                                buffer,
                                STATUS_BUFFER_SIZE,
                                100);
        ...
        public void open(UsbDeviceConnection connection) throws IOException {
            ...
            try {
                ...
                if (mDevice.getDeviceClass() == 0x02) {
                    ...
                    try {
                        Method getRawDescriptorsMethod = mConnection.getClass()
                                .getMethod("getRawDescriptors");
                        byte[] rawDescriptors = (byte[]) getRawDescriptorsMethod
                                .invoke(mConnection);
                        ...
                        Log.e(TAG,
                                "An unexpected exception occurred while trying "
                ...
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread != null) {
                        try {
                            ...
                            Log.w(TAG,
                                    "An error occurred while waiting for status read thread",
                                    e);
                        }
                    }
                }
        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            synchronized (mReadBufferLock) {
                ...
                int numBytesRead = mConnection.bulkTransfer(mReadEndpoint,
                        mReadBuffer,
