...
class FalconViewDrawFileFeatureDefinition : public FeatureDefinition2 {
   public:
    FalconViewDrawFileFeatureDefinition(const String &name, GeometryPtr geometry, StylePtr style,
...
class FalconViewDrawFileContent : public FeatureDataSource2::Content {
   public:
    FalconViewDrawFileContent(DatabasePtr &&database, TAK::Engine::Port::String name) NOTHROWS;
    ~FalconViewDrawFileContent() NOTHROWS override;
   ...
   private:
    ...
    int64_t item_num_;
    std::unique_ptr<FalconViewDrawFileFeatureDefinition, void (*)(const FalconViewDrawFileFeatureDefinition *)> feature_definition_;
};

class FalconViewLocalPointFeatureDefinition : public FeatureDefinition2 {
   public:
    FalconViewLocalPointFeatureDefinition(const String &id, const String &desc, double lat, double lng, int elev,
...
class FalconViewLocalPointContent : public FeatureDataSource2::Content {
   public:
    FalconViewLocalPointContent(DatabasePtr &&database, TAK::Engine::Port::String name) NOTHROWS;
    ~FalconViewLocalPointContent() NOTHROWS override;
   ...
   private:
    ...
    std::unique_ptr<FalconViewLocalPointFeatureDefinition, void (*)(const FalconViewLocalPointFeatureDefinition *)> feature_definition_;
};
}  // namespace
...
TAKErr FalconViewFeatureDataSource::parse(FeatureDataSource2::ContentPtr &content, const char *file) NOTHROWS {
    ...
    TAKErr code = MsAccessDatabaseFactory_create(db, info);
    ...
    if (drw == 0) {
        content = FeatureDataSource2::ContentPtr(new FalconViewDrawFileContent(std::move(db), name),
                                                 Memory_deleter_const<FeatureDataSource2::Content, FalconViewDrawFileContent>);
        return TE_Ok;
    } else if (lpt == 0) {
        content = FeatureDataSource2::ContentPtr(new FalconViewLocalPointContent(std::move(db), name),
                                                 Memory_deleter_const<FeatureDataSource2::Content, FalconViewLocalPointContent>);
        return TE_Ok;
    }
...
const char *FalconViewFeatureDataSource::getName() const NOTHROWS { return "falconview"; }

int FalconViewFeatureDataSource::parseVersion() const NOTHROWS { return 1; }

namespace {
FalconViewDrawFileContent::FalconViewDrawFileContent(DatabasePtr &&database, TAK::Engine::Port::String name) NOTHROWS
    : complete_(false),
...
FalconViewDrawFileContent::~FalconViewDrawFileContent() NOTHROWS {}

const char *FalconViewDrawFileContent::getType() const NOTHROWS { return "drw"; }
const char *FalconViewDrawFileContent::getProvider() const NOTHROWS { return "falconview"; }

TAKErr FalconViewDrawFileContent::moveToNextFeature() NOTHROWS {
...
TAKErr FalconViewDrawFileContent::parsePolyline() NOTHROWS {
    TAKErr code;
    int64_t item_num(0);
    ...
    std::unique_ptr<FalconViewDrawFileFeatureDefinition, void (*)(const FalconViewDrawFileFeatureDefinition *)> result(
        new FalconViewDrawFileFeatureDefinition(feature_name_.c_str(),
        ...
        Memory_deleter_const<FalconViewDrawFileFeatureDefinition>);
...
TAKErr FalconViewDrawFileContent::parseEllipse() NOTHROWS {
    TAKErr code;
    int64_t item_num(0);
...
TAKErr FalconViewDrawFileContent::parseText() NOTHROWS {
    TAKErr code;
    int64_t item_num(0);
...
TAKErr FalconViewDrawFileContent::parseBullseye() NOTHROWS {
    TAKErr code;
    int64_t item_num(0);
...
TAKErr FalconViewDrawFileContent::parseRectangle() NOTHROWS {
    TAKErr code;
    int64_t item_num(0);
...
TAKErr FalconViewDrawFileContent::parseAxis() NOTHROWS {
    TAKErr code;
    int64_t item_num(0);
    ...
    if (isnan(ll.altitude)) ll.altitude = 0;
    ll.altitude += 100;
    ...
    std::vector<GeoPoint2> geo_points;
    {
        ...
        // Left side, pos->tgt
        // pos + perp*Width
        scratch.multiply(width, &scratch);
        ...
        // (pos + dir*shaftlen*0.25) + (perp*Width*0.7)
        scratch = Vector4<double>(dir);
        scratch.multiply(shaft_length * 0.25, &scratch);
        scratch2 = Vector4<double>(perp);
        scratch2.multiply(width * 0.7, &scratch2);
        scratch.add(&pos, &scratch);
        scratch.add(&scratch2, &scratch);
        ecef->inverse(&geo_scratch, Point2<double>(scratch.x, scratch.y, scratch.z));
        geo_points.push_back(geo_scratch);

        // (pos + dir*shaftlen*0.5) + (perp*Width*0.5)
        scratch = Vector4<double>(dir);
        scratch.multiply(shaft_length * 0.5, &scratch);
        scratch2 = Vector4<double>(perp);
        scratch2.multiply(width * 0.5, &scratch2);
        scratch.add(&pos, &scratch);
        scratch.add(&scratch2, &scratch);
        ecef->inverse(&geo_scratch, Point2<double>(scratch.x, scratch.y, scratch.z));
        geo_points.push_back(geo_scratch);
        ...
        // Head
        if (cross) {
            // (pos + dir*shaftlen) - (perp*Width*0.5)
            scratch = Vector4<double>(dir);
            scratch.multiply(shaft_length, &scratch);
            scratch2 = Vector4<double>(perp);
            scratch2.multiply(width * 0.5, &scratch2);
            scratch.add(&pos, &scratch);
            scratch.subtract(&scratch2, &scratch);
            ecef->inverse(&geo_scratch, Point2<double>(scratch.x, scratch.y, scratch.z));
            geo_points.push_back(geo_scratch);

            // (pos + dir*shaftlen) - (perp*Width)
            scratch = Vector4<double>(dir);
            scratch.multiply(shaft_length, &scratch);
            scratch2 = Vector4<double>(perp);
            scratch2.multiply(width, &scratch2);
            scratch.add(&pos, &scratch);
            scratch.subtract(&scratch2, &scratch);
            ecef->inverse(&geo_scratch, Point2<double>(scratch.x, scratch.y, scratch.z));
            geo_points.push_back(geo_scratch);
        ...
        // pos - perp*Width
        scratch = Vector4<double>(perp);
        scratch.multiply(width, &scratch);
        pos.subtract(&scratch, &scratch);
        ecef->inverse(&geo_scratch, Point2<double>(scratch.x, scratch.y, scratch.z));
        geo_points.push_back(geo_scratch);
    }

    atakmap::feature::Geometry *geometry = createGeometry(geo_points, true);
...
void FalconViewDrawFileContent::parseGeneral(const DataType &data_type, std::unique_ptr<atakmap::util::AttributeSet> &attributes) NOTHROWS {
    switch (data_type) {
        case DataType::Label: {
            std::string data = getData();
            feature_name_ = data;
...
void FalconViewDrawFileContent::parseColor(int *fore_color_rgb, int *back_color_rgb) NOTHROWS {
    std::string data = getData();
...
TAKErr FalconViewDrawFileContent::parseLatitudeLongitudeFromString(double *latitude, double *longitude) NOTHROWS {
    std::string data = getData();
...
TAKErr FalconViewDrawFileContent::getRgbColorFromColorData(int *rgb, int color) NOTHROWS {
    switch (color) {
        case 0:  // Black
            *rgb = 0xFF000000;
            return TE_Ok;
...
atakmap::feature::LineString FalconViewDrawFileContent::createLineString(const std::vector<GeoPoint2> &geo_points) {
...
atakmap::feature::Geometry *FalconViewDrawFileContent::createGeometry(const std::vector<GeoPoint2> &geo_points, bool closed) {
    atakmap::feature::Geometry *geometry;
...
atakmap::feature::Style *FalconViewDrawFileContent::createStyle(LineStyle line_style, FillStyle fill_style, int stroke_rgb, int fill_rgb,
...
atakmap::feature::Style *FalconViewDrawFileContent::createStrokeStyle(LineStyle line_style, int stroke_rgb, float stroke_width) {
    atakmap::feature::Style *style;
    switch (line_style) {
        case LineStyle::Dot:
...
std::string FalconViewDrawFileContent::getData() NOTHROWS {
...
TAKErr FalconViewDrawFileContent::moveToNextFeatureSet() NOTHROWS {
...
TAKErr FalconViewDrawFileContent::get(FeatureDefinition2 **feature) const NOTHROWS {
    *feature = feature_definition_.get();
    return TE_Ok;
}
TAKErr FalconViewDrawFileContent::getFeatureSetName(TAK::Engine::Port::String &name) const NOTHROWS {
    name = feature_set_name_.c_str();
    return TE_Ok;
}
TAKErr FalconViewDrawFileContent::getFeatureSetVisible(bool *visible) const NOTHROWS {
    *visible = true;
    return TE_Ok;
}
TAKErr FalconViewDrawFileContent::getMinResolution(double *value) const NOTHROWS {
    *value = 156543.034;
    return TE_Ok;
}
TAKErr FalconViewDrawFileContent::getMaxResolution(double *value) const NOTHROWS {
    *value = MAX_RESOLUTION;
    return TE_Ok;
}
TAKErr FalconViewDrawFileContent::getVisible(bool *visible) const NOTHROWS {
    *visible = true;
    return TE_Ok;
}

FalconViewDrawFileFeatureDefinition::FalconViewDrawFileFeatureDefinition(const String &name, GeometryPtr geometry, StylePtr style,
                                                                         std::unique_ptr<atakmap::util::AttributeSet> attributes) NOTHROWS
    : name_(name),
...
TAKErr FalconViewDrawFileFeatureDefinition::getRawGeometry(RawData *value) NOTHROWS {
    value->object = geometry_.get();
    return TE_Ok;
}

FeatureDefinition2::GeometryEncoding FalconViewDrawFileFeatureDefinition::getGeomCoding() NOTHROWS { return GeomGeometry; }

AltitudeMode FalconViewDrawFileFeatureDefinition::getAltitudeMode() NOTHROWS { return AltitudeMode::TEAM_ClampToGround; }

double FalconViewDrawFileFeatureDefinition::getExtrude() NOTHROWS { return 0.0; }

TAKErr FalconViewDrawFileFeatureDefinition::getName(const char **value) NOTHROWS {
    *value = name_;
    return TE_Ok;
}

FeatureDefinition2::StyleEncoding FalconViewDrawFileFeatureDefinition::getStyleCoding() NOTHROWS { return StyleStyle; }

TAKErr FalconViewDrawFileFeatureDefinition::getRawStyle(RawData *value) NOTHROWS {
    value->object = style_.get();
    return TE_Ok;
}

TAKErr FalconViewDrawFileFeatureDefinition::getAttributes(const atakmap::util::AttributeSet **value) NOTHROWS {
    *value = attributes_.get();
    return TE_Ok;
}

TAKErr FalconViewDrawFileFeatureDefinition::get(const Feature2 **feature) NOTHROWS {
    feature_.reset();
...
FalconViewLocalPointContent::FalconViewLocalPointContent(DatabasePtr &&database, TAK::Engine::Port::String name) NOTHROWS
    : feature_set_count_(0),
      database_(std::move(database)),
      cursor_(nullptr, nullptr),
      feature_set_name_(name),
...
FalconViewLocalPointContent::~FalconViewLocalPointContent() NOTHROWS {}

const char *FalconViewLocalPointContent::getType() const NOTHROWS { return "lpt"; }
const char *FalconViewLocalPointContent::getProvider() const NOTHROWS { return "falconview"; }

TAKErr FalconViewLocalPointContent::moveToNextFeature() NOTHROWS {
    ...
    std::unique_ptr<FalconViewLocalPointFeatureDefinition, void (*)(const FalconViewLocalPointFeatureDefinition *)> result(
        new FalconViewLocalPointFeatureDefinition(string_id, string_desc, lat, lng, elev, string_icon),
        Memory_deleter_const<FalconViewLocalPointFeatureDefinition>);
...
TAKErr FalconViewLocalPointContent::moveToNextFeatureSet() NOTHROWS {
    // RWI - Only 1 Feature Set
    if (feature_set_count_ == 0) {
        feature_set_count_++;
        return TE_Ok;
...
TAKErr FalconViewLocalPointContent::get(FeatureDefinition2 **feature) const NOTHROWS {
    *feature = feature_definition_.get();
    return TE_Ok;
}
TAKErr FalconViewLocalPointContent::getFeatureSetName(TAK::Engine::Port::String &name) const NOTHROWS {
    name = feature_set_name_;
    return TE_Ok;
}
TAKErr FalconViewLocalPointContent::getFeatureSetVisible(bool *visible) const NOTHROWS {
    *visible = true;
    return TE_Ok;
}
TAKErr FalconViewLocalPointContent::getMinResolution(double *value) const NOTHROWS {
    *value = 156543.034;
    return TE_Ok;
}
TAKErr FalconViewLocalPointContent::getMaxResolution(double *value) const NOTHROWS {
    *value = MAX_RESOLUTION;
    return TE_Ok;
}
TAKErr FalconViewLocalPointContent::getVisible(bool *visible) const NOTHROWS {
    *visible = true;
    return TE_Ok;
}

FalconViewLocalPointFeatureDefinition::FalconViewLocalPointFeatureDefinition(const String &id, const String &desc, double lat, double lng,
                                                                             int elev, const String &icon) NOTHROWS
    : name_(id),
...
TAKErr FalconViewLocalPointFeatureDefinition::getRawGeometry(RawData *value) NOTHROWS {
    value->object = geometry_.get();
    return TE_Ok;
}

FeatureDefinition2::GeometryEncoding FalconViewLocalPointFeatureDefinition::getGeomCoding() NOTHROWS { return GeomGeometry; }

AltitudeMode FalconViewLocalPointFeatureDefinition::getAltitudeMode() NOTHROWS { return AltitudeMode::TEAM_Absolute; }

double FalconViewLocalPointFeatureDefinition::getExtrude() NOTHROWS { return -1; }

TAKErr FalconViewLocalPointFeatureDefinition::getName(const char **value) NOTHROWS {
    *value = name_;
    return TE_Ok;
}

FeatureDefinition2::StyleEncoding FalconViewLocalPointFeatureDefinition::getStyleCoding() NOTHROWS { return StyleEncoding::StyleStyle; }

TAKErr FalconViewLocalPointFeatureDefinition::getRawStyle(RawData *value) NOTHROWS {
    value->object = style_.get();
    return TE_Ok;
}

TAKErr FalconViewLocalPointFeatureDefinition::getAttributes(const atakmap::util::AttributeSet **value) NOTHROWS {
    *value = &attributes_;
    return TE_Ok;
}

TAKErr FalconViewLocalPointFeatureDefinition::get(const Feature2 **feature) NOTHROWS {
    feature_.reset();
    TAKErr code = Feature_create(feature_, *this);
    TE_CHECKRETURN_CODE(code);
...
String FalconViewLocalPointFeatureDefinition::getIconUri(const String &icon_name) NOTHROWS {
