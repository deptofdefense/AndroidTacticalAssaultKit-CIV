...
TAKErr KMLDriverDefinition2::getStyle(Port::String &value) NOTHROWS
{
    ...
    if (TAK::Engine::Port::String_endsWith(filePath, "kmz"))
      {
        const char* symbolStart(std::strstr(value, "SYMBOL("));
        ...
        if (symbolStart)
        {
            ...
            if (idStart)
            {
                ...
                if (!colon || colon > uriEnd)
                {
                    ...
                    //
                    // XXX-- Clang needs static_cast or the call is ambiguous
                    value[static_cast<int>(uriStart - value - 1)] = '\0';
                    stringBuilder << value << ':';
                    if (*uriStart == '"')
                    {
                        ++uriStart;
                    ...
                    stringBuilder << "zip://" << filePath << "!/" << uriStart;
