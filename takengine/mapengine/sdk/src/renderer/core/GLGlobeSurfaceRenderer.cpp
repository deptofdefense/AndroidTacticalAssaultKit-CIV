...
bool GLGlobeSurfaceRenderer::updateTiles(ProgramCounter &pc, const std::vector<std::size_t> &updateIndices, const bool allowInterrupt, const int64_t limit) NOTHROWS
{
    ...
    while(pc.pc < updateIndices.size()) {
        ...
        const TerrainTile &tile = *updateContext.resolveTiles[updateIndices[pc.pc++]];
...
void GLGlobeSurfaceRenderer::draw() NOTHROWS
{
    ...
    // RENDER MEGATEXTURE
    TerrainTileRenderContext ctx = GLTerrainTile_begin(owner.renderPasses[0u].scene, GLTerrainTile_getColorShader(owner.context, owner.renderPasses[0].drawSrid));
    const char *shader = "???";
    {
        auto shaders = GLTerrainTile_getColorShader(owner.context, owner.renderPasses[0].drawSrid);
