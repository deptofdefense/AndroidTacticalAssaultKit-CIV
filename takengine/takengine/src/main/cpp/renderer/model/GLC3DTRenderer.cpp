...
public:
    ...
    TAKErr processB3DMMesh(const std::shared_ptr<const Mesh>& mesh, GLBatchBuilder& batchBuilder, TAK::Engine::Core::RenderContext& ctx, const std::shared_ptr<Scene>& scene, const String& baseURI, const char* URI, bool isStreaming, const char* anchorPath) NOTHROWS;
    TAKErr processB3DMChildren(SceneNode& node, GLBatchBuilder& batchBuilder, TAK::Engine::Core::RenderContext& ctx, const std::shared_ptr<Scene>& scene, const String& baseURI, const char* URI, bool isStreaming, const char* anchorPath) NOTHROWS;
...
namespace {
        ...
        last_state_(NONE)
    {
        ...
        if (boundingVolume.type == C3DTVolume::Region || boundingVolume.type == C3DTVolume::RegionAux) {
            ...
            transform_.transform(&center, center);
    ...
    void GLB3DM::onTileContentLoaded(GLC3DTTile* parentTile) NOTHROWS {
        if (parentTile) {
...
TAKErr GLC3DTRenderer::LoaderImpl::processB3DMMesh(const std::shared_ptr<const Mesh>& mesh, GLBatchBuilder& batchBuilder, TAK::Engine::Core::RenderContext& ctx, const std::shared_ptr<Scene>& scene, const String& baseURI, const char* URI, bool isStreaming, const char* anchorPath) NOTHROWS {
...
TAKErr GLC3DTRenderer::LoaderImpl::processB3DMChildren(SceneNode& node, GLBatchBuilder& batchBuilder, TAK::Engine::Core::RenderContext& ctx, const std::shared_ptr<Scene>& scene, const String& baseURI, const char* URI, bool isStreaming, const char* anchorPath) NOTHROWS {
    ...
    while (iter->get(item) == TE_Ok) {
        if (item) {
            ...
            if (item->hasMesh()) { // should be the case
                std::shared_ptr<const Mesh> mesh;
                item->loadMesh(mesh);
                code = processB3DMMesh(mesh, batchBuilder, ctx, scene, baseURI, URI, isStreaming, subAnchorPath.c_str());
            } else if (item->hasChildren()) {
                code = processB3DMChildren(*item, batchBuilder, ctx, scene, baseURI, URI, isStreaming, subAnchorPath.c_str());
            }
        }
        iter->next();
        ++nodeIndex;
...
TAKErr GLC3DTRenderer::LoaderImpl::processB3DM(TAK::Engine::Renderer::Core::GLMapRenderable2Ptr& output, TAK::Engine::Core::RenderContext& ctx, ScenePtr &scenePtr, const String& baseURI, const char* URI, bool isStreaming) NOTHROWS {
    ...
    code = processB3DMChildren(node, batchBuilder, ctx, scene, baseURI, URI, isStreaming, "");
