...
namespace TAK {
    namespace Engine {
        namespace Renderer {
            ...
            inline Util::TAKErr Skirt_createIndices(Util::MemBuffer2 &skirtIndices, const int mode, const Util::MemBuffer2 *edgeIndices, const std::size_t edgeCount, const IndexType nextIdx) NOTHROWS
            {
            ...
            inline Util::TAKErr Skirt_createVertices(Util::MemBuffer2 &vertices, const int mode, const std::size_t stride_, const Util::MemBuffer2 *edgeIndices, const std::size_t count, const VertexType height) NOTHROWS
            {
                Util::TAKErr code(Util::TE_Ok);
                switch(mode) {
                    case GL_TRIANGLE_STRIP :
                ...
                for (std::size_t i = 0; i < count; i++) {
                    ...
                    code = srcVertices.position(basePos+index*stride);
                    TE_CHECKBREAK_CODE(code);
