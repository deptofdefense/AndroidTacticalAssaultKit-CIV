...
TAKErr OGR_FeatureDataSource2::parse(ContentPtr& content, const char* filePath) NOTHROWS {
    ...
    try {
        LegacyContentPtr legacyContent(new OGR_Content(filePath, areaThreshold), Memory_deleter_const<feature::FeatureDataSource::Content>);
        LegacyAdapters_adapt(content, std::move(legacyContent));
    } catch (...) {
        return TE_Err;
    }
    return TE_Ok;
