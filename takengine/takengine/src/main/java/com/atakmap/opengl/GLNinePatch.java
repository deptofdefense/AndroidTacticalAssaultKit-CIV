...
public class GLNinePatch {
    ...
    public void draw(float x, float y, float z, float width, float height, boolean textured) {
        ...
        if (textured && this.textureId != 0) {
            this.drawTexture(x, y, z, width, height);
        } else
            this.drawTriangleFan(x, y, z, width, height);
    }

    private void drawTriangleFan(float x, float y, float z, float width, float height) {
        if (this.lastTriangleX != x ||
                this.lastTriangleY != y ||
                this.lastTriangleZ != z ||
                this.lastTriangleWidth != width ||
        ...
        GLES20FixedPipeline.glVertexPointer((z == 0f) ? 2 : 3,
                GLES20FixedPipeline.GL_FLOAT, 0, this.triangleVerts);
    ...
    private void drawTexture(float x, float y, float z, float width, float height) {
        if (this.lastTextureX != x ||
                this.lastTextureY != y ||
                this.lastTextureZ != z ||
                this.lastTextureWidth != width ||
        ...
        GLES20FixedPipeline.glVertexPointer((z == 0.0f) ? 2 : 3, GLES20FixedPipeline.GL_FLOAT, 0, this.texVerts);
        GLES20FixedPipeline.glTexCoordPointer(2, GLES20FixedPipeline.GL_FLOAT, 0, this.texCoords);

        GLES20FixedPipeline.glDrawElements(GLES20FixedPipeline.GL_TRIANGLES, 54, GLES20FixedPipeline.GL_UNSIGNED_SHORT, TEX_INDICES);
