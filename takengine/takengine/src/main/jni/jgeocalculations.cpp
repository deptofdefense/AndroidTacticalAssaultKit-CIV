...
JNIEXPORT jobject JNICALL Java_com_atakmap_coremap_maps_coords_GeoCalculations_pointAtDistance__DDDDDI
  (JNIEnv *env, jclass clazz, jdouble lat, jdouble lng, jdouble azimuth, jdouble distance, jdouble inclination, jint flags)
{
    const GeoPoint2 a(lat, lng);
    GeoPoint2 result = GeoPoint2_pointAtDistance(GeoPoint2(lat, lng), azimuth, distance, inclination, false);
    return NewGeoPoint(*env, result.latitude, result.longitude);
}
